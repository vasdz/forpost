...
процесс при остановке первого. `/health` остаётся `503 not_ready`: запуск
...
Не печатайте значения переменных и не коммитьте их. В карточке сработки в
«Журналах наблюдений» нажмите «Войти как диспетчер ОДС». Подписанное demo-
...
Тесты используют временный детерминированный snapshot и dependency override
...
неизменность снимка, ошибки 401/503/500 и ненулевой exit code при отказе.

CLI читает уже подготовленный processed-снимок, не обращается к `data/raw`,
не создаёт заявку или SQLite и не требует отдельно запущенных серверов.
Два маршрута — `/api/availability` и `/api/v1/topology` — чередуются между
пользователями. Все ответы вне 2xx и таймауты считаются ошибками. По умолчанию
секрет создаётся только внутри процесса; значение в JSON не попадает.
...
Отсутствующий/невалидный snapshot даёт ошибку topology, а не успешный прогон.
...
от serving. У `rejected` нет test-метрик. Валидный отчёт не превращает
