...
export default function OverviewPage() {
  return <><PageHeader eyebrow="Локальный контур" title="Наблюдаемая оперативная обстановка" description="Сводка ограниченного локального обезличенного снимка: журнальные события, каналы и объекты. Прогнозы, риски и рекомендации здесь не формируются." />
    <LocalSituationGate>{(snapshot) => {
      const alarmCount = snapshot.events.filter((event) => event.isAlarm === true).length;
      return <div className="page-grid">
        <ObservationMetric label="Наблюдаемые события" value={snapshot.events.length} note="Записи из локального снимка" />
        ...
        <section className="col-span-12"><h2 className="mb-3 font-heading text-xl font-semibold">Журнал наблюдаемых событий</h2><ObservedEventTable events={snapshot.events} ariaLabel="Журнал наблюдаемых событий" /></section>
