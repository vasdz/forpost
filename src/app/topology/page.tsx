...
function SchematicDiagram({ topology }: { topology: TopologyFeatureCollection }) {
  ...
  return <Card className="overflow-hidden p-0"><div className="flex items-center justify-between border-b border-[var(--color-border)] px-3 py-2"><span className="text-xs text-[var(--color-text-muted)]"><Move size={14} className="mr-1 inline" />Перетаскивайте, нажимайте узлы; масштаб {Math.round(scale * 100)}%</span><div className="flex gap-1"><Button size="sm" variant="ghost" aria-label="Уменьшить масштаб" onClick={() => setScale((v) => Math.max(.5, v - .2))}><Minus size={15} /></Button><Button size="sm" variant="ghost" aria-label="Увеличить масштаб" onClick={() => setScale((v) => Math.min(4, v + .2))}><Plus size={15} /></Button><Button size="sm" variant="ghost" aria-label="Сбросить вид схемы" onClick={reset}><RotateCcw size={15} /></Button></div></div>{selected && <div className="grid grid-cols-1 gap-2 border-b border-[var(--color-border)] bg-[var(--color-panel)] px-4 py-3 text-xs md:grid-cols-4"><span><b>Объект:</b> {selected.properties.dispatcherName}</span><span><b>ID:</b> {selected.properties.objectId}</span><span><b>Тип:</b> {selected.properties.objectKind}</span><span><b>Родитель:</b> {selected.properties.parentId ?? 'корневой узел'}</span></div>}<div className="h-[430px] touch-none cursor-grab overflow-hidden bg-[var(--color-panel-2)] active:cursor-grabbing" onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); setDrag({ x: e.clientX, y: e.clientY, panX: pan.x, panY: pan.y }); }} onPointerMove={(e) => { if (drag) setPan({ x: drag.panX + e.clientX - drag.x, y: drag.panY + e.clientY - drag.y }); }} onPointerUp={() => setDrag(null)} onWheel={(e) => { e.preventDefault(); setScale((v) => Math.max(.5, Math.min(4, v + (e.deltaY < 0 ? .15 : -.15)))); }}><svg role="img" aria-labelledby="topology-title topology-desc" viewBox={box} className="h-full w-full origin-center transition-transform duration-100" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})` }}><title id="topology-title">Условная схема объектов</title><desc id="topology-desc">Нажмите узел, чтобы прочитать имя, ID, тип и родительский объект. Координаты не являются географическими.</desc>{topology.features.flatMap((feature) => feature.geometry.coordinates.map((line, index) => <polyline key={`${feature.id}-${index}`} points={line.map(([x, y]) => `${x},${y}`).join(' ')} fill="none" stroke="var(--color-data)" strokeWidth="0.08" vectorEffect="non-scaling-stroke" />))}{nodes.map(({ feature, point }) => <g key={feature.id} onClick={(event) => { event.stopPropagation(); setSelectedId(feature.id); }} className="cursor-pointer"><circle cx={point[0]} cy={point[1]} r="0.16" fill={selectedId === feature.id ? 'var(--color-warning)' : 'var(--color-data)'} vectorEffect="non-scaling-stroke" /><title>{`${feature.properties.dispatcherName} · ${feature.properties.objectId}`}</title>{(selectedId === feature.id || (scale >= 1.6 && feature.properties.parentId === null)) && <text x={point[0] + .2} y={point[1] - .2} fill="var(--color-text)" fontSize="0.55" stroke="var(--color-panel-2)" strokeWidth="0.08" paintOrder="stroke">{feature.properties.dispatcherName}</text>}</g>)}</svg></div><p className="border-t border-[var(--color-border)] px-4 py-2 text-xs text-[var(--color-text-muted)]">Точки - объекты из снимка; линии - иерархические связи. Наведите курсор или нажмите точку, чтобы увидеть подпись.</p></Card>;
}
