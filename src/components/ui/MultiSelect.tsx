...
export function MultiSelect({ label, options, value, onChange }: MultiSelectProps) {
  ...
  return <div className="relative grid gap-1 text-xs text-[var(--color-text-muted)]">
    <span>{label}</span>
    <button type="button" aria-expanded={isOpen} aria-controls={listId} onClick={() => setIsOpen((open) => !open)} className="control-surface flex h-10 items-center justify-between rounded-[8px] px-3.5 text-left text-sm text-[var(--color-text)]">
      <span>{label} — выбрано: {value.length}</span><ChevronDown size={16} aria-hidden="true" />
    </button>
    {isOpen && <div id={listId} role="group" aria-label={label} className="glass-surface absolute top-full z-20 mt-2 w-full p-2">
