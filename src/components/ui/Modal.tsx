...
export function Modal({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#05070d]/80 p-4" onMouseDown={onClose}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} onKeyDown={onKeyDown} onMouseDown={(event) => event.stopPropagation()} className="surface max-h-[calc(100vh-32px)] w-full max-w-3xl overflow-y-auto rounded-[6px] p-6 md:p-8"><header className="mb-6 flex items-start justify-between gap-4"><h2 id={titleId} className="font-heading text-xl font-semibold tracking-[-0.02em]">{title}</h2><Button variant="ghost" size="sm" onClick={onClose} aria-label="Закрыть окно"><X size={17} aria-hidden="true" /></Button></header>{children}</div></div>;
}
