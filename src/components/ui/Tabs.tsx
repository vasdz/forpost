...
export function Tabs({ tabs, defaultTabId, value, onValueChange }: { tabs: Tab[]; defaultTabId?: string; value?: string; onValueChange?: (id: string) => void }) {
  ...
  return <div><div role="tablist" aria-label="Разделы" className="inline-flex gap-1 rounded-[8px] border border-[var(--color-border)] bg-[var(--color-panel-2)] p-1">{tabs.map((tab, index) => <button key={tab.id} id={`${baseId}-${tab.id}`} role="tab" type="button" aria-selected={tab.id === activeId} aria-controls={`${baseId}-panel-${tab.id}`} tabIndex={tab.id === activeId ? 0 : -1} onKeyDown={(event) => onKeyDown(index, event)} onClick={() => setActiveId(tab.id)} className={cn('rounded-[6px] border px-3 py-1.5 text-xs font-medium transition-colors', tab.id === activeId ? 'border-[var(--color-border)] bg-[var(--color-panel)] text-[var(--color-text)]' : 'border-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text)]')}>{tab.label}</button>)}</div><div id={`${baseId}-panel-${activeTab.id}`} role="tabpanel" aria-labelledby={`${baseId}-${activeTab.id}`} className="pt-5">{activeTab.content}</div></div>;
}
