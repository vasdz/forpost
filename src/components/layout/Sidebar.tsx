...
export function Sidebar() {
  ...
  return <aside className={cn('glass-surface-subtle sticky top-0 z-40 flex h-screen shrink-0 flex-col rounded-none border-y-0 border-l-0 transition-[width] duration-200 ease-out', 'w-[var(--active-sidebar-width)]')}>
    ...
    <nav aria-label="Основная навигация" className="flex-1 space-y-1.5 p-3 pt-5">{navigation.map((item) => {
      ...
      const link = <Link href={item.href} aria-label={item.label} aria-current={active ? 'page' : undefined} className={cn('flex h-10 items-center gap-3 rounded-[7px] border-l-2 px-3 text-[13px] transition-colors', active ? 'border-l-[var(--color-data)] bg-[var(--color-hover)] text-[var(--color-text)]' : 'border-l-transparent text-[var(--color-text-muted)] hover:bg-[var(--color-panel-2)] hover:text-[var(--color-text)]')}><Icon size={17} strokeWidth={1.7} aria-hidden="true" /><span className={cn(sidebarCollapsed ? 'sr-only' : 'max-md:sr-only')}>{item.label}</span></Link>;
