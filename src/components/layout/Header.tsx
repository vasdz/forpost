...
import { fetchPredictionFeed } from '@/data/predictionsClient';
...
export function getModelLabel(status: 'loading' | 'ready' | 'unavailable'): string {
  if (status === 'loading') return 'ML-модель: проверка';
  return status === 'ready' ? 'ML-модель: доступна' : 'ML-модель: недоступна';
}
...
export function Header() {
  ...
  useEffect(() => {
    let active = true;
    void fetchPredictionFeed().then((feed) => {
      if (active) setModelStatus(feed.status === 'ready' ? 'ready' : 'unavailable');
    });
    return () => { active = false; };
  }, []);
  ...
  return <header className="surface-subtle fixed left-[var(--active-sidebar-width)] right-0 top-0 z-30 flex h-[var(--header-height)] items-center justify-between rounded-none border-x-0 border-t-0 px-4 md:px-6">
    ...
    <div className="flex items-center gap-3"><span className="hidden items-center gap-2 rounded-[6px] border border-[var(--color-border)] bg-[var(--color-panel-2)] px-3 py-1.5 text-[11px] text-[var(--color-text-muted)] md:inline-flex"><BrainCircuit size={14} className={modelStatus === 'ready' ? 'text-[var(--color-success)]' : 'text-[var(--color-warning)]'} aria-hidden="true" />{getModelLabel(modelStatus)}</span><span className="hidden text-[11px] text-[var(--color-text-dim)] lg:inline">{sourceLabel}</span><Button variant="ghost" size="sm" onClick={toggleTheme} aria-label={theme === 'dark' ? 'Включить светлую тему' : 'Включить тёмную тему'}>{theme === 'dark' ? <Sun size={17} aria-hidden="true" /> : <Moon size={17} aria-hidden="true" />}</Button></div>
  </header>;
}
