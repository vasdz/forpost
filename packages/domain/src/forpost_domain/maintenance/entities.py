...
class MaintenanceOrder(BaseModel):
    ...
    risk_category: str = Field(
        ..., description="Категория риска (sensor_failure, fire_risk и т.д.)"
    )
    ...
    created_at: datetime
    generated_by_model_version: str = Field(
        ..., description="Версия модели, сгенерировавшей заявку"
    )
...
class MaintenanceGenerationRequest(BaseModel):
    ...
    risk_prediction_ids: list[str] = Field(
        default_factory=list, description="Список ID прогнозов для преобразования"
    )
    auto_approve: bool = Field(
        default=False, description="Автоматически утверждать созданные заявки"
    )
